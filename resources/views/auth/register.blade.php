            ...
            <div class="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 transition-all duration-1000 ease-out opacity-0 translate-y-5" id="register-form-container">
                <div class="w-full max-w-md">
                    ...
                    <form id="register-form" class="space-y-4 md:space-y-6" method="POST" action="{{ route('register') }}">
                        ...
                        {{-- Base input classes --}}
                        @php
                            // UPDATED: Added 'placeholder:text-start' and REMOVED 'ps-10'
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                {{-- Input requires ps-10 for the icon --}}
                                <input type="text" id="name" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" placeholder="{{ __('auth_pages.name_placeholder') }}"
                                       class="{{ $inputClasses }} @error('name') {{ $errorClasses }} @enderror ps-10">
                            </div>
                            @error('name')
                            ...
                            <div class="relative">
                                <div class="absolute inset-y-0 start-0 flex items-center ps-3.5 pointer-events-none">
                                 ...
                                 <input type="email" id="email" name="email" value="{{ old('email') }}" required autocomplete="username" placeholder="{{ __('auth_pages.email_placeholder') }}"
                                       class="{{ $inputClasses }} @error('email') {{ $errorClasses }} @enderror ps-10">
                            </div>
                            @error('email')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            {{-- The input no longer needs the icon wrapper or padding classes --}}
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="government_id" name="government_id" value="{{ old('government_id') }}" required autocomplete="off" placeholder="{{ __('auth_pages.government_id_placeholder') }}"
                                       class="{{ $inputClasses }} @error('government_id') {{ $errorClasses }} @enderror ps-10">
                            </div>
                            @error('government_id')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="password" id="password" name="password" required autocomplete="new-password" placeholder="{{ __('auth_pages.password_placeholder') }}"
                                       class="{{ $inputClasses }} @error('password') {{ $errorClasses }} @enderror ps-10">
                            </div>
                            @error('password')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                            @enderror
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (phoneInput && registerForm) {
            ...
            registerForm.addEventListener('submit', function(e) {
                ...
                phoneInput.value = iti.getNumber(); // set full phone number
